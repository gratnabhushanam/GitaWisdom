...
export default function Quiz() {
  ...
  const fetchQuestions = async () => {
    try {
      ...
      const fetched = Array.isArray(response.data) ? response.data : response.data.questions || [];
      setQuestions(fetched.length > 0 ? fetched : fallbackQuestions);
    } catch {
    ...
    return (
      <div className="min-h-screen bg-[#06101E] pt-24 pb-12 px-4 flex items-center justify-center relative overflow-hidden">
        <div className="text-center">
          ...
          <button onClick={fetchQuestions} className="mt-4 bg-[#FFD700] text-[#06101E] px-6 py-2 rounded-full font-bold hover:brightness-110">
            Reload
          </button>
        </div>
      </div>
          ...
          <div className="animate-fade-in-up">
            ...
            <div className="flex justify-between items-center border-b border-white/10 pb-6 mb-8">
              <h1 className="text-3xl font-serif font-bold text-[#FFD700]">
                {questions[currentQuestion]?.category || 'Gita Challenge'}
              </h1>
